...
export function SummaryReport({ analysisData, solutionName }: SummaryReportProps) {
  ...
  const handleExportData = async () => {
    ...
    if (window.toolboxAPI) {
      try {
        await window.toolboxAPI.utils.saveFile(filename, content);
  ...
  const calculateComplexityScore = (): number => {
    const avgDependencies = analysisData.stats.assetCount > 0 
      ? analysisData.stats.linkCount / analysisData.stats.assetCount 
      : 0;
            ...
            <tbody>
              {topAssets.map(asset => (
                <tr key={asset.assetId}>
                  ...
                  <td><strong>{asset.linksTo.length + (asset.linkedBy?.length || 0)}</strong></td>
                </tr>
              ))}
            </tbody>
          </table>
