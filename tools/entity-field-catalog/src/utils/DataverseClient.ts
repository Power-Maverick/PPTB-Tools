...
export class DataverseClient {
  ...
  async listSolutions(): Promise<DataverseSolution[]> {
    try {
      const entitySetName =
        await window.dataverseAPI.getEntitySetName("solution");
  ...
  async fetchSolutionEntities(
    ...
    try {
      ...
      const componentEntitySetName =
        await window.dataverseAPI.getEntitySetName("solutioncomponent");
  ...
  private async fetchEntities(entityIds: string[]): Promise<DataverseEntity[]> {
    ...
    const entities = entityIds.map(async (id) => {
      ...
      const entityAttributes =
        await window.dataverseAPI.getEntityRelatedMetadata(
      ...
      return {
        ...
        displayName:
          entityMetadata.DisplayName?.LocalizedLabels?.[0]?.Label ||
          entityMetadata.LogicalName,
