...
export class ExportUtil {
     ...
     */
    static async export(entities: DataverseEntity[], solutionName: string, customColumns: CustomColumn[] = []): Promise<void> {
     ...
     */
    private static async exportToExcel(entities: DataverseEntity[], solutionName: string, customColumns: CustomColumn[] = []): Promise<void> {
    ...
    private static addEntitySummaryWorksheet(workbook: ExcelJS.Workbook, entities: DataverseEntity[]): void {
    ...
    private static addEntityFieldWorksheets(workbook: ExcelJS.Workbook, entities: DataverseEntity[], customColumns: CustomColumn[] = []): void {
        const usedNames = new Set<string>(["Entities"]);

        entities.forEach((entity) => {
            const sheetName = this.sanitizeWorksheetName(entity.displayName || entity.logicalName, usedNames);
            ...
            const baseColumns: Partial<ExcelJS.Column>[] = [
                ...
                { header: "Required Level", key: "requiredLevel", width: 18 },
            ...
            const yesNoColumns = [5, 6]; // isPrimaryId, isPrimaryName (1-indexed column numbers)
            const requiredLevelColumn = 7;
            ...
            if (totalFields > 0) {
                ...
                const requiredLevels = ["None", "Recommended", "ApplicationRequired", "SystemRequired"];
                worksheet.dataValidations.add(`${this.getColumnLetter(requiredLevelColumn)}2:${this.getColumnLetter(requiredLevelColumn)}${totalFields + 1}`, {
                    type: "list",
                    allowBlank: true,
                    formulae: [`"${requiredLevels.join(",")}"`],
                    showErrorMessage: true,
                    errorStyle: "warning",
                    errorTitle: "Invalid Required Level",
                    error: "Choose a valid Dataverse required level value.",
                });
            }
        });
    }
    ...
    private static applyAlternatingRowFill(worksheet: ExcelJS.Worksheet, headerRowIndex = 1): void {
    ...
    private static sanitizeWorksheetName(name: string, usedNames: Set<string>): string {
